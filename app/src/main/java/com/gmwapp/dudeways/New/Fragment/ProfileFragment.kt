...
import com.gmwapp.dudeways.databinding.FragmentProfileBinding
...
import com.gmwapp.dudeways.New.UpdateProfileActivity
...
//        Glide.with(requireActivity()).load(session.getData(Constant.COVER_IMG)).placeholder(R.drawable.placeholder_bg).into(binding.ivCover)
...
//                Glide.with(requireActivity()).load(session.getData(Constant.COVER_IMG)).placeholder(R.drawable.placeholder_bg).into(binding.ivCover)
    ...
    private fun redirectToLogin(context: Context) {
        ...
        val intent = Intent(context, com.gmwapp.dudeways.New.LoginActivity::class.java)
